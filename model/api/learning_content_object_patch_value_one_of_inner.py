...
LEARNINGCONTENTOBJECTPATCHVALUEONEOFINNER_ONE_OF_SCHEMAS = ["LearningContentObject", "bool", "float", "int", "str"]


class LearningContentObjectPatchValueOneOfInner(BaseModel):
    """
    LearningContentObjectPatchValueOneOfInner
    """
    # data type: str
    ...
    actual_instance: Optional[Union[LearningContentObject, bool, float, int, str]] = None
    one_of_schemas: List[str] = Literal["LearningContentObject", "bool", "float", "int", "str"]
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = LearningContentObjectPatchValueOneOfInner.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in LearningContentObjectPatchValueOneOfInner with oneOf schemas: LearningContentObject, bool, float, int, str. Details: " + ", ".join(
                    error_messages))
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in LearningContentObjectPatchValueOneOfInner with oneOf schemas: LearningContentObject, bool, float, int, str. Details: " + ", ".join(
                    error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when deserializing the JSON string into LearningContentObjectPatchValueOneOfInner with oneOf schemas: LearningContentObject, bool, float, int, str. Details: " + ", ".join(
                    error_messages))
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into LearningContentObjectPatchValueOneOfInner with oneOf schemas: LearningContentObject, bool, float, int, str. Details: " + ", ".join(
                    error_messages))
        else:
            return instance
